...
def deposito(valor):
    global saldo
    if pode_realizar_operacao():
        saldo += valor
        lista_de_depositos.append((valor, datetime.now()))
...
def saque(valor):
    ...
    if saques >= LIMITE_SAQUES_POR_DIA:
        ...
        voltar_menu()
        return
    elif pode_realizar_operacao():
        saldo -= valor
        saques += 1
        lista_de_saques.append((valor, datetime.now()))
...
def extrato():
    ...
    for operacao_realizada in lista_de_operacao_realizada_saque_ou_deposito_copia:
        if operacao_realizada == 1:
            print(f"- R$ {lista_de_saques_copia[indice_saques][0].__format__('0.2f')} às {lista_de_saques_copia[indice_saques][1].strftime('%d/%m/%Y %H:%M:%S')}")
            indice_saques += 1
        elif operacao_realizada == 2:
            print(f"+ R$ {lista_de_depositos_copia[indice_depositos][0].__format__('0.2f')} às {lista_de_depositos_copia[indice_depositos][1].strftime('%d/%m/%Y %H:%M:%S')}")
            indice_depositos += 1
...
def pode_realizar_operacao():
    if len(lista_de_saques) + len(lista_de_depositos) > LIMITE_DE_OPERACOES_POR_DIA:
        return False
    return True
